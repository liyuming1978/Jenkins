...
public class LocalExploreActivity extends ListActivity {
	...
	//private String 			mRoot = "/storage/sdcard1/";
	private String			mCurrentDir = mRoot;
	private TextView 		mTextViewLocation;
	private File[]			mFiles;
	
	static String[] exts = {"hevc", "hm91", "hm10", "bit", "hvc", "h265", "265", "flv", "ts", "m3u8","mp4"};
   ...
   @Override
	...
	private void getDirectory(String dirPath) {
		try {
			...
			MessageBox.show(this, "Sorry", dirPath+" get fail.");
			ex.printStackTrace(); 
		}
	}
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
	        case MENU1:
	        {
	        	...
	        	try {
		        	File file = new File(Environment.getExternalStorageDirectory().getPath() + "/.hevdeocoder/history.txt");
		        	if (file.exists()) {
					...
					public void onClick(DialogInterface arg0, int arg1) {
						...
						try {
							if (input.startsWith("rtsp://") || input.startsWith("http://") 
								...
								File file = new File(Environment.getExternalStorageDirectory().getPath() + "/.hevdecoder/history.txt");
								if (!file.exists()) {
									new File(Environment.getExternalStorageDirectory().getPath() + "/.hevdecoder").mkdir();
									file.createNewFile();
								}
