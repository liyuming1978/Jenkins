...
static const char* const kClassPathName = "pku/shengbin/hevdecoder/NativeMediaPlayer";
...
#if LOOP_PLAY
	if (!exit_decode_thread) {
		if (avformat_seek_file(ff_fmt_ctx, ff_vid_strm_idx, LONG_LONG_MIN, 0, LONG_LONG_MAX, AVSEEK_FLAG_FRAME) < 0) {
...
static int ffmpeg_prepare(int threadNumber)
{
	...
	//ff_vid_dec_ctx->pix_fmt =AV_PIX_FMT_YUV420P;  //liyuming mark
...
{
    jclass clazz;
    clazz = env->FindClass("pku/shengbin/hevdecoder/NativeMediaPlayer");
    if (clazz == NULL) {
